...
// USB CDC configuration (ESP32-C3)
...
typedef enum {
    ...
    // Legacy motor configuration commands (for backwards compatibility)
    CMD_SET_MOTOR_POLES        = 0x20,  // Set motor poles (payload: uint8)
    CMD_SET_MOTOR_PULLEY       = 0x21,  // Set motor pulley teeth (payload: uint8)
    CMD_SET_WHEEL_PULLEY       = 0x22,  // Set wheel pulley teeth (payload: uint8)
    CMD_SET_WHEEL_SIZE         = 0x23,  // Set wheel diameter in mm (payload: uint16)
