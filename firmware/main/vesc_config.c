...
#ifdef CONFIG_TARGET_LITE
  err = nvs_set_u8(nvs_handle, NVS_KEY_INVERT_THROTTLE,
                   (uint8_t)config->invert_throttle);
  if (err != ESP_OK)
    goto cleanup;
#endif
...
void vesc_config_update_motor(uint8_t motor_poles, uint16_t gear_ratio_x1000,
  ...
  if (motor_poles != current_motor_config.motor_poles ||
    ...
    ESP_LOGI(
        TAG,
        "Motor config from VESC: poles=%d, gear_ratio=%.3f, wheel_diam=%dmm",
...
int32_t vesc_config_get_speed(const vesc_config_t *config) {
  ...
  // Convert ERPM to mechanical RPM: ERPM / pole_pairs = ERPM / (motor_poles /
  // 2)
  float pole_pairs = (float)config->motor_poles / 2.0f;
  float rpm = (float)erpm / pole_pairs;
  float gear_ratio = (float)config->gear_ratio_x1000 /
                     1000.0f; // gear_ratio from BLE (already scaled)

  float wheel_circumference_m =
      (float)config->wheel_diameter_mm / 1000.0f * M_PI;
